...
uint32_t get_delta(float* fft_out, uint32_t* delta_pos, uint32_t len){
    float freq_step = (float)ADC_CLK/CIC_SAMPLE_RATE/FFT_SAMPLE_LEN;
    ...
    for(uint32_t i = 0; i < fft_end; ++i)
    	aver += fft_out[i];
    aver /= fft_end;
    uint32_t width = (uint32_t)(2.5e3/freq_step);
    // 从 MIN_FREQ 开始搜索（5kHz），而不是写死 15kHz
    uint32_t search_start = (uint32_t)(MIN_FREQ / freq_step);
    if (search_start < 1) search_start = 1;

    for(uint32_t i = search_start; i < fft_end; ++i){
        if(fft_out[i] > 3e-3){
            // 修复越界：当 pos==0 时跳过 delta_pos[pos-1] 比较
        	if(pos == 0 || delta_pos[pos-1] < i-width){
        			delta_pos[pos] = i;
					++pos;
        	}
